...
public class BudgetAnnotator extends CybionAbstractAnnotator {
  ...
  private static final String SIMPLE_BUDGET_REGEX = "(VAT|(B|b)udget|(A|a)ward):\\s*\\({0,1}\\s*(USD|Û|euro|$|pound|Euro|EURO|EUR|£|Pound|Dollar|Dollars){0,1}\\s*\\){0,1}\\s*(\\d{1,}\\.{0,1}\\,{0,1}\\s{0,1}){1,}\\s*\\({0,1}\\s*(USD|Û|euro|$|pound|Euro|EURO|EUR|£|Pound|Dollar|Dollars){0,1}\\s*\\){0,1}\\s*(per award){0,1}";
