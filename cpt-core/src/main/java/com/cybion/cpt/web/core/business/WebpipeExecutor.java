...
import it.chi.webpipe.nr.model.Tuple;
import it.chi.webpipe.nr.model.impl.Serializer;
import it.chi.webpipe.nr.model.impl.XMLSerializer;
...
public class WebpipeExecutor {
	...
	public List<Page> fireCrawling() {
		...
		try {
			...
			out1.write(crawlingSpec);out1.flush();out1.close();
...
//				Thread.currentThread().sleep(2000);
		...
		} catch (Exception e1) {logger.error("ERRORE SCRITTURA FILES",e1);}
...
//				Reader reader2 = ResourceUtils.getResourceAsReader(wrapPath);
				...
				WrappingSpecification ws = wsr.read(reader2);
				ws.setModel(model);
